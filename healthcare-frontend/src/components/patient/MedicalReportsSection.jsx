...
import { FileText, Upload, Download, FilePlus } from "lucide-react";
...
const MedicalReportsSection = ({ userId }) => {
  ...
  const handleDownload = (reportId, fileName) => {
    api
      .get(`/users/reports/${reportId}`, { responseType: "blob" })
      .then((response) => {
        ...
        {loading ? (
          ...
          <div className="pat-empty-state">
            <div className="pat-empty-state__icon">
              <FilePlus size={48} color="var(--border-focus)" />
            </div>
            <div className="pat-empty-state__text">
              No medical reports uploaded yet
            </div>
            <div className="pat-empty-state__sub">
              Upload your lab results, scans, or prescriptions for easy access
            </div>
          </div>
        ) : (
