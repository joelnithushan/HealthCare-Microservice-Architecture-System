...
import { validateNIC, validateLankanMobile, validateEmail, validateDOB } from '../utils/validations';
...
const RegisterPatient = () => {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    const errs = validatePatient(formData);
    setFieldErrors(errs);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // Step 3: Register after OTP is verified
      try {
        await api.post('/auth/register', formData);
        toast.success('Patient Account created successfully! Redirecting to login…');
        setShowOtpModal(false);
        setTimeout(() => navigate('/login'), 2000);
      } catch (regErr) {
        toast.error(regErr.response?.data?.message || 'Registration failed. Email or NIC might be taken.');
        setShowOtpModal(false);
      }
    } catch (err) {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    setLoading(true);
    try {
      const response = await api.post('/auth/google', { token: credentialResponse.credential, role: 'PATIENT' });
    ...
    <div style={styles.pageWrapper}>
      <div style={styles.cardContainer}>
        <div style={styles.brandPanel}>
          ...
          <h2 style={styles.brandTitle}>Patient Portal</h2>
          <p style={styles.brandSub}>
            Register securely using your Sri Lankan national identity credentials to organize all your health profiles safely.
          </p>
        </div>

        <div style={styles.formPanel}>
          <h2 style={styles.formTitle}>Patient Registration</h2>
          <p style={styles.formSub}>Looking for Doctor Registration? <Link to="/register/doctor" style={styles.switchLink}>Click Here</Link></p>
          ...
          <form onSubmit={handleRegister} noValidate style={styles.formGrid}>
            <div style={{...styles.formGroup, gridColumn: 'span 2'}}>
              ...
              <input type="text" name="name" className="flat-input" value={formData.name} onChange={handleChange} placeholder="e.g. Nimal Perera" style={styles.input} />
              {fieldErrors.name && <span style={styles.fieldError}>{fieldErrors.name}</span>}
            </div>
            
            <div style={styles.formGroup}>
              <label style={styles.label}>Email Address</label>
              <input type="email" name="email" className="flat-input" value={formData.email} onChange={handleChange} placeholder="nimal.p@gmail.com" style={styles.input} />
            ...
            <div style={styles.formGroup}>
              ...
              <input type="text" name="nic" className="flat-input" value={formData.nic} onChange={handleChange} placeholder="e.g. 961234567V" style={styles.input} />
              {fieldErrors.nic && <span style={styles.fieldError}>{fieldErrors.nic}</span>}
            </div>

            <div style={styles.formGroup}>
              <label style={styles.label}>Mobile Number</label>
              <input type="text" name="mobileNumber" className="flat-input" value={formData.mobileNumber} onChange={handleChange} placeholder="e.g. +94771234567" style={styles.input} />
            ...
            <div style={{gridColumn: 'span 2'}}>
              <button type="submit" className="flat-btn" style={styles.submitBtn} disabled={loading}>
                {loading ? 'Sending Verification Code…' : 'Create Patient Account'}
              </button>
            </div>
          </form>
