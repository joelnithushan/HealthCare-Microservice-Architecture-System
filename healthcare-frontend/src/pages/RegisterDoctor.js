...
import { validateNIC, validateLankanMobile, validateEmail, validateSLMC } from '../utils/validations';

const validateDoctor = (data) => {
  const errors = {};
  if (!data.name.trim()) errors.name = 'Full name is required.';
  else if (!/^[a-zA-Z\s.]+$/.test(data.name)) errors.name = 'Name can only contain letters, spaces, and periods.';
  ...
  else if (!/(?=.*[A-Z])(?=.*[0-9])/.test(data.password)) errors.password = 'Password needs at least 1 uppercase and 1 number.';
  ...
  if (!data.specialization.trim()) errors.specialization = 'Please select a specialization.';
  if (!data.hospitalAttached.trim()) errors.hospitalAttached = 'Please specify primary hospital.';
  return errors;
};
...
const RegisterDoctor = () => {
  const [formData, setFormData] = useState({
    ...
    nic: '', mobileNumber: '+94', dob: '', gender: '',
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      try {
        ...
        toast.success('Doctor Application submitted successfully! Redirecting to login…');
        setShowOtpModal(false);
        setTimeout(() => navigate('/login'), 2000);
      } catch (regErr) {
        toast.error(regErr.response?.data?.message || 'Registration failed. SLMC or Email might be registered.');
        setShowOtpModal(false);
      }
    } catch (err) {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    setLoading(true);
    try {
      const response = await api.post('/auth/google', { token: credentialResponse.credential, role: 'DOCTOR' });
      ...
      <div style={styles.cardContainer}>
        <div style={styles.brandPanel}>
          ...
          <p style={styles.brandSub}>
            Join Sri Lanka's fastest growing digital hospital network. Register with your SLMC number to start accepting patient appointments.
          </p>
        </div>

        <div style={styles.formPanel}>
          <h2 style={styles.formTitle}>Physician Registration</h2>
          <p style={styles.formSub}>Looking for Patient Registration? <Link to="/register/patient" style={styles.switchLink}>Click Here</Link></p>
          ...
          <form onSubmit={handleRegister} noValidate style={styles.formGrid}>
            <div style={{...styles.formGroup, gridColumn: 'span 2'}}>
              ...
              <input type="text" name="name" className="flat-input" value={formData.name} onChange={handleChange} placeholder="e.g. Dr. Sanjeewa Fernando" style={styles.input} />
              {fieldErrors.name && <span style={styles.fieldError}>{fieldErrors.name}</span>}
            </div>
            
            <div style={styles.formGroup}>
              <label style={styles.label}>Email Address</label>
              <input type="email" name="email" className="flat-input" value={formData.email} onChange={handleChange} placeholder="sanjeewa.f@gmail.com" style={styles.input} />
            ...
            <div style={styles.formGroup}>
              ...
              <input type="text" name="slmcNumber" className="flat-input" value={formData.slmcNumber} onChange={handleChange} placeholder="e.g. 23456" style={styles.input} />
            ...
            <div style={styles.formGroup}>
              ...
              <input type="text" name="hospitalAttached" className="flat-input" value={formData.hospitalAttached} onChange={handleChange} placeholder="e.g. Nawaloka Hospital" style={styles.input} />
            ...
            <div style={styles.formGroup}>
              ...
              <input type="text" name="mobileNumber" className="flat-input" value={formData.mobileNumber} onChange={handleChange} placeholder="e.g. +94712345678" style={styles.input} />
            ...
            <div style={styles.formGroup}>
              ...
              <input type="text" name="nic" className="flat-input" value={formData.nic} onChange={handleChange} placeholder="e.g. 198512345678" style={styles.input} />
          ...
          <p style={styles.switchText}>
            Already have a doctor account? <Link to="/login" style={styles.switchLink}>Sign in</Link>
          </p>
        </div>
      </div>
      ...
      {showOtpModal && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            <button onClick={() => setShowOtpModal(false)} style={styles.modalClose}>&times;</button>
            ...
            <div style={styles.otpContainer} onPaste={handleOtpPaste}>
              {otpDigits.map((digit, idx) => (
                <input
                  ...
                  style={{
                    ...styles.otpInput,
                    borderColor: otpError ? '#ef4444' : digit ? '#0ea5e9' : '#e2e8f0',
            ...
            <p style={styles.resendText}>
              ...
              {resendCooldown > 0 ? (
                ...
                <button onClick={handleResendOtp} style={styles.resendBtn} disabled={otpLoading}>
                  Resend Code
                </button>
              )}
            </p>
